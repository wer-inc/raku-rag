...
stg で `この根拠でもう少し詳しく`, `手順だけ見る`, `注意点を確認`, `根拠を確認する`
のいずれを押しても、根拠不足の確認依頼に倒れることをユーザー確認で再検出した。
原因は、follow-up 検索文に前回回答の UI 用整形文(`前回回答:`, `参照範囲:`, `不明点:` など)を
混ぜていたため、元質問と引用文書への再検索が不安定になったこと。

対応方針:

1. follow-up 検索文は元質問、追加依頼、引用 document_id に絞る。
2. `担当者` などの handoff 文言を follow-up 検索文に入れない。
3. `根拠を確認する` は再検索せず、直前のフィルタ済み citation summary を返す。
4. 回帰テストで UI 用整形文が検索文に混ざったら失敗させる。
...
1. `details` quick reply を前回質問・前回引用文書IDを含むフォローアップ検索文に変換する。
