...
- [x] 承認後の公開経路(issue 0019 フォローアップ) — レビュー詳細の「承認済み — 正式に公開できます」誇大文言を実体化: `POST /v1/manufacturing/drafts/{id}/publish`(reviewer/admin ゲート、承認済みのみ・二重公開は409)で承認ドラフトを決定論 markdown 化し `ingest_manufacturing` 経由で approved+effective 文書(`pub_<artifact_id>`、draft provenance 付き)として知識化。UI は未公開時「ナレッジとして公開」ボタン(ConfirmDialog)/公開後は published_at+公開文書リンクの「公開済み」表示。`draft.published` をハッシュチェーン監査に記録。詳細: `issues/0019-no-publish-path-after-approval.md`(Resolved)
