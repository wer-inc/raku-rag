...
| §18 | 運用メトリクス（§18.1 status別レート+provider分布+fallback率+**provider失敗率+pages_processed+route分布** / §18.2 reason別+quarantine率+**seal/vertical/drawing個別ページ数+review_overturn_rate** / §18.3 stage別 latency p50/p95（実計測）+ provider別 call数・cost（config）+**cost_per_page** / §18.4 回答引用の extraction 品質分布・review承認chunk使用率・高リスク引用eligible・高リスク引用不備ブロック数、OK回答時に `MetricsRecorder` へ live emit + **manufacturing overlay の HTTP境界でも live emit**） | live（§18.4 の retrieval hit rate/citation exactness/user feedback by doc type は既存 eval/observability 側；§18.3 の queue待ち・worker memory は infra 側） | `ingestion_quality`: `extraction_quality_stats`(+`provider_failure_rate`) + `extraction_latency_cost_stats`（route_trace `latency_ms` を doc単位集計、+`pages_processed`/`route_distribution`/`cost_per_page`/artefact別ページ数）+ `review_overturn_stats`（audit event の accept↔reject 反転検出）+ `services/ingestion_cost.provider_cost_model`（`RAKU_INGEST_COST_MODEL`）+ `answer_citation_quality_signals`/`aggregate_answer_quality_signals`（citation.metadata から §18.4 projection、`services/answer.py` OK path + `apps/answer-service/server.py` manufacturing overlay の両方で emit）; docling/OCR/VLM 実呼び出しを計時（`RouteTraceStep.latency_ms`）; `GET …/metrics.latency_cost`/`.review` |
