...
| **B** Postgres/pgvector/RLS contract + parity | スキーマ/migration、Document/Chunk/ACL repository 契約、**RLS分離**、pgvector retrieval、**既存security gateのPostgres版**、多テナント release soak | `infra/docker-compose.yml`(Postgres+pgvector) | 秒〜十数秒 | **全 push/PR で必須**（+ local `gate.sh b`） | compose を**run毎に1回**起動（test毎でない）／session-scoped fixture／contract粒度（最小データ）／並列 |
...
4. **path-based gating は重い外周に限定**。Tier B は release gate なので全 push/PR で常時実行する。
   Tier C/D は引き続き変更領域に応じて実行し、E は毎PRで回さない。
...
- **Tier B** は全 push/PR の release gate として常時実行する。**Tier C/D** は PR で、
  `infra/docker-compose.yml` を `services:` or `docker compose up` で起動して回す（path filter で
  該当変更時のみ）。既存 `ci.yml`（python/security/node ジョブ）は Tier B/D の素地として発展・統合。
