...
>
> **`[x]` の意味（2026-06-26 整合, analyze I1）**: T001–T072 の `[x]` は **stdlib MVP 層（in-memory Python `ManufacturingSystem`）での完了**を指す。**HTTP API（NestJS controller / answer-service `/internal/manufacturing/*`）と production 永続（DataUsePolicy テーブル等）は当初 `[x]` 時点では未実装**で、2026-06-20 の design-vs-impl 監査が GAP-F05（API First 未達）/ GAP-F08（DataUsePolicy 非永続）として検出 → **2026-06-22 に migration 0010 + NestJS `ManufacturingController` で解消済み**（下記 Gap Backlog 参照）。「全タスク `[x]`」≠「全層 production 実装済み」。
