...
- [X] T117 [P1] Add load-test scenario for large corpus / many tenants / long documents / high concurrency with p50/p95/p99 and retrieval/rerank/generation breakdown. Sandbox smoke + reusable harness landed in `tests/benchmarks/load_harness.py` and `tests/integration/test_load_smoke.py`; real deployed p50/p95/p99 remains a release-environment gate.
- [X] T118 [P1] Verify authorization/tenant/metadata filter pushdown with EXPLAIN/query-plan checks and fail readiness when post-filter-only search is detected. Static query-shape/index contract plus Tier-B runtime script landed in `tests/contract/test_explain_gate.py` and `scripts/postgres-explain-gate.sh`; production-scale EXPLAIN remains a release-environment gate.
- [X] T119 [P1] Move LLM-as-judge and citation deep validation out of the synchronous production path except for profile-gated sampling/eval. The answer hot path performs one generation call only; citation revalidation is deterministic ACL/tombstone/document-state checking, and eval faithfulness uses deterministic CI scoring. Regression coverage pins extra profile flags to no extra synchronous LLM calls in `tests/integration/test_rag_performance_caps.py`.
