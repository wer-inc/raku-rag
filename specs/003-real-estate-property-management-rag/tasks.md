...
- [X] GAP-F14 [func] domain-table RLS UNTESTED across 003/004/005/006 (FR-RE-062/063, FR-IM-071/072, SC-RE-003/SC-IM-005): closed locally with a Tier-B live-Postgres test and gate wiring. `tests/postgres/test_domain_table_rls.py` inserts tenant A/B rows into `industry_profiles`, `real_estate_properties`, and `investment_funds` as `raku_app`, switches `app.current_tenant_id`, and asserts tenant B rows are invisible to tenant A. `scripts/gate.sh b` now applies pgvector init plus 0002/0003/0004/0005/0006 before discovering `tests/postgres`, so the domain RLS test runs in the Docker/Postgres Tier-B gate. Local developer runs without Postgres/domain tables skip the test.
