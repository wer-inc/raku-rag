...
- [X] GAP-F13 [func] financial/regulated extension policies declared but NOT applied at runtime (FR-IM-025/026/030/032/054): closed locally. Investment runtime now reads the declared regulated policies instead of relying only on hardcoded coarse nets: `RegulatedActivityPolicy.prohibited_ai_actions` blocks explicit prohibited actions and restricted actions force human review; `ComplianceReviewPolicy.review_states` is mapped to runtime statuses (`changes_requested` → `changes_required`, `compliance_approved` → `approved`) without weakening the existing approval prerequisite; `DisclosureEvidencePolicy.required_source_document_types` governs marketing source selection and persisted disclosure validation; `MarketingMaterialPolicy` continues to govern per-statement contradiction/risk-disclosure checks; `AdviceBoundaryPolicy` continues to govern advice gating OR-merged with the keyword net. Pinned by `tests/industry/test_investment_api.py` and `tests/industry/test_framework.py`.
